...
function StringMask(options) {
	...
	this._maskParsed = StringMask._parseMask(this._options.mask);
	this._offsets = StringMask._countOffsets(this._maskParsed);
	console.log('StringMask', this);
}

StringMask._parseMask = function getPlaceholder(mask) {
	var maskParsed = [];
...
StringMask.prototype.unmaskIndex = function(index) {
	console.log('unmaskIndex', index, this._offsets[index]);
